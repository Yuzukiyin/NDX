    ...
    <Layout>
      <div className="space-y-10">
        ...
        <div className="flex items-end justify-between border-b border-gray-200 pb-6">
          <div>
            <h1 className="text-5xl font-black text-gray-900 tracking-tight mb-3">概览</h1>
            <p className="text-gray-500 text-lg">您的投资组合表现</p>
          </div>
        </div>
          ...
          {stats.map((stat, index) => {
            ...
            return (
              <motion.div
                ...
                className={`relative bg-gradient-to-br ${stat.gradient} rounded-3xl p-8 border border-gray-200/60 shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden group`}
              >
                {/* Background Pattern */}
                ...
                <div className="relative z-10">
                  {/* Icon */}
                  <div className={`${stat.iconBg} w-14 h-14 rounded-2xl flex items-center justify-center mb-5 group-hover:scale-110 transition-transform duration-300`}>
                  ...
                  {/* Value */}
                  <p className={`text-3xl font-black ${stat.iconColor} tracking-tight`}>
                    {stat.value}
                  </p>
                </div>
              </motion.div>
        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-3xl font-bold text-gray-900 tracking-tight">持仓基金</h2>
            <Link 
              to="/funds"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {funds.slice(0, 4).map((fund, index) => (
              <motion.div
                ...
                className="bg-white/80 backdrop-blur-sm rounded-3xl p-8 border border-gray-200/60 shadow-lg hover:shadow-2xl transition-all duration-300"
              >
                {/* Fund Header */}
                <div className="flex items-start justify-between mb-6">
                  <div className="flex-1">
                    <h3 className="font-bold text-gray-900 text-xl mb-2 tracking-tight">
                      {fund.fund_name}
                    </h3>
                    <p className="text-sm text-gray-500 font-mono">{fund.fund_code}</p>
                  </div>
                  <div className={`
